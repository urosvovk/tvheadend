                            ...
                            uint8_t i_cmd, int *pi_capmt_size )
{
    ...
    {
        /*uint16_t i_pid = pmtn_get_pid( p_es );*/ /*[urosv]*/
        j++;

        /*if ( demux_PIDIsSelected( i_pid ) ) *//*[urosv] Ignore selection, because structures used by this function are not alive. TVHeadend has its own. TODO: switch to tvh dvbtables for additional checking.*/
        {
            b_has_es = true;
            b_has_ca = b_has_ca
    ...
    while ( (p_es = pmt_get_es( p_pmt, j )) != NULL )
    {
        ...
        /*if ( !demux_PIDIsSelected( i_pid ) )
            continue;*/ /*[urosv] Ignore selection, because structures used by this function are not alive. TVHeadend has its own. TODO: switch to tvh dvbtables.*/
...
/****************************************************************************
 * CheckForCompatibleScramblingSystem
 *
 * Find if the CAM module can descramble the service given in PMT block.
 *  It is based on caid's checking as is done in CAPMTBuild(.) function.
 ****************************************************************************/
bool CheckForCompatibleScramblingSystem(uint8_t *p_pmt)
{
  int i_session_id;

  /*First find the CA session*/
  for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ ) {
      if ( p_sessions[i_session_id - 1].i_resource_id == RI_CONDITIONAL_ACCESS_SUPPORT )
      {
        /* ... then compare caids */
